/**
  * Created by rtripathi on 22 Feb 2017
  */
...
object IPApp {
  ...
  val IMAGE_CATEGORIES = List("luxury","suv", "sports", "electric","military","royal", "sedan","convertible","coupe","jeep")
  ...
  def classifyImage(sc: SparkContext, path: String): Double = {
    ...
    println(s"Predicting test image path : " + path)
    val model = KMeansModel.load(sc, IPSettings.KMEANS_PATH)
    val vocabulary = ImageUtils.vectorsToMat(model.clusterCenters)
    ...
    val p = nbModel.predict(histogram)
    println(s"Predicting test image : " + p.toInt)
    println(s"Predicting test image : " + IMAGE_CATEGORIES(p.toInt))
  ...
  def main(args: Array[String]) {
    ...
    val images = sc.wholeTextFiles(s"${IPSettings.INPUT_DIR}/*/*.jpeg")
    ...
    val testImages = sc.wholeTextFiles(s"${IPSettings.TEST_INPUT_DIR}/*/*.jpeg")
    ...
    testImagesArray.foreach(f => {
      ...
      println(s"Predicting test image : " + cat)
    ...
    println("Accuracy obtained is : "+accuracy)
